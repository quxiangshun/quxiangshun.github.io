...
\r
仓库内 \`tech-share/pgsql/sync-netbox-db-manual.sh\` 将**源** \`192.168.0.1:5432\` 的 \`netbox_src\`（用户 \`qxs_src\`）同步到**目标** \`192.168.0.2:5433\` 的 \`netbox_target\`（用户 \`qxs_target\`），分三步手动执行。默认 dump 路径为 \`/tmp/netbox.dump\`；若本机 PostgreSQL 客户端不在默认路径，可设置 \`PG_BIN\`（指向包含 \`pg_dump\` / \`pg_restore\` 的目录）。\r
\r
在 \`tech-share/pgsql\` 目录下执行：\r
\r
\`\`\`bash\r
